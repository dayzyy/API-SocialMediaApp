...
export default function Friends(){
  ...
  const is_following = _ => {
    if (!searchedUser || !user) return

    return searchedUser.friends.some(friend => friend.id === user.id)
  }
      ...
      <div className="flex flex-col gap-4">
        ...
        {searchedUser && (searchedUser.id === -1121 ?
          ...
          <div className="flex gap-2 justify-between gap-1 hover:bg-gray-50  p-2 cursor-pointer">
            ...
            <button className={`p-2  rounded-md  ${!is_following() ? 'bg-blue-600 text-white'  : 'border'}`}>{is_following() ? 'following' : 'follow'}</button>
          </div>)
        }
      </div>

      <div className="w-full md:w-[45rem] flex flex-col gap-4">
        {
          user && user.friends.map(friend => {
            return (
              <div key={friend.id} className="w-full  flex justify-between  hover:bg-gray-50  p-2 cursor-pointer">
                ...
                <button className={"p-2  border rounded-md"}>following</button>
              </div>
            )
          })
        }
